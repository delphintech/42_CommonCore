...
/*   Updated: 2024/11/28 14:40:55 by dabouab          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*use_rest(char *str, char *rest)
{
	...
	if (rest[0] != '\0')
	{
		...
		str = ft_strljoin(str, &rest[0], i + 2);
		ft_strlcpy(&rest[0], &rest[i + 1], ft_strlen(&rest[0]) - i);
	}	
	return (str);
}
...
char	*get_next_line(int fd)
{
	...
	while (nbread != 0 && rest[0] == '\0' && has_eol(str) == 0)
	{
		...
		if ((size_t)i != ft_strlen(buf))
			nbread = 0;
		str = ft_strljoin(str, buf, i + 2);
		ft_strlcpy(&rest[0], &buf[i + 1], ft_strlen(buf) - i);
	}
	if (str[0] == 0)
